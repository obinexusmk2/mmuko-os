...
#   image    stage1 + stage2 + runtime    → build/mmuko-os.img  (1.44 MB raw)
#
...
# Runtime sector budget (32 sectors × 512 = 16384 bytes max)
RUNTIME_SECTORS      := 32

# ============================================================
# REPL (mmuko-repl / RIFT shell)
# ============================================================
REPL_DIR      := repl
...
PRIMARY_PSC      := $(PSC_BOOT_DIR)/mmuko-boot.psc
CODEGEN_STAMP    := $(BUILD)/mmuko_codegen.stamp
SPEC_FILE        := MMUKO-OS.txt
...
.PHONY: all dirs install-deps firmware firmware-cpp bios-firmware bios-firmware-cpp \
        phase1-ui flat-bin qemu-flat boot image run repl run-repl \
        cython-build cython-develop run-ui verify tree clean help codegen ring

# ============================================================
# Default: build everything
# ============================================================
all: dirs codegen firmware firmware-cpp bios-firmware phase1-ui boot image flat-bin repl
	@echo ""
	@echo "======================================="
...
dirs:
	@$(PY) scripts/ensure_dirs.py $(OBJ_DIR) $(LIB_DIR) $(BUILD) \
		$(OBJ_DIR)/kernel $(OBJ_DIR)/firmware $(OBJ_DIR)/ui $(OBJ_DIR)/repl
...
$(CODEGEN_STAMP): $(SPEC_FILE) $(PRIMARY_PSC) $(wildcard $(PSC_DIR)/*.psc)
	@echo "[CODEGEN] Generating MMUKO-OS derived sources..."
	$(PY) $(CODEGEN_SCRIPT) --root . --spec MMUKO-OS.txt \
		--primary mmuko-boot/pseudocode/mmuko-boot.psc \
		--pseudocode-dir mmuko-boot/pseudocode
	@touch $(CODEGEN_STAMP)
...
# ============================================================
# Disk image (1.44 MB raw fixed-sector layout — stage1 + stage2 + runtime)
# ============================================================
.FORCE:
image: $(DISK_IMG)

$(DISK_IMG): .FORCE | dirs
	@echo "[IMAGE] Writing $(DISK_IMG) (1.44 MB raw fixed-sector layout)..."
	@$(PY) -c "\
...
help:
	...
	@echo ""
	@echo "ALL TARGETS:"
	@echo "  make all                  firmware + boot + image + bios + phase1-ui + flat-bin + repl"
	...
	@echo "  make image                write raw sectors into build/mmuko-os.img"
