...
int nsigii_transmitter_init(nsigii_channel_t* channel);
int nsigii_transmitter_encode_message(const uint8_t* raw_content, size_t content_len,
                                       nsigii_packet_t* packet);
int nsigii_transmitter_transmit(const nsigii_channel_t* channel, 
                                 const nsigii_packet_t* packet);

/* Receiver (Channel 1) */
int nsigii_receiver_init(nsigii_channel_t* channel);
int nsigii_receiver_decode_packet(const uint8_t* encoded_data, size_t data_len,
                                   nsigii_packet_t* packet);
int nsigii_receiver_receive(nsigii_channel_t* channel, nsigii_packet_t* packet);
int nsigii_receiver_forward_to_verifier(const nsigii_packet_t* packet);

/* Verifier (Channel 2) */
int nsigii_verifier_init(nsigii_channel_t* channel);
int nsigii_verifier_verify_packet(const nsigii_packet_t* packet,
                                   nsigii_verification_result_t* result);
int nsigii_verifier_emit_consensus_message(const nsigii_packet_t* verified_packet,
